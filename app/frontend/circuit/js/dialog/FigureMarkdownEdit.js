...
export default class FigureMarkdownEdit {
  ...
  show(view) {
    ...
    var splash = $(
      ...
      '</div>' +
      ' <div class="header">' +
      '<span class="left">Formula Editor (<a target="_blank" href="https://en.wikipedia.org/wiki/Computation_tree_logic">CTL* syntax </a>)</span>' +
      '<span class="right">Computation Tree Visualisation</span></div>' +
      ' <img title="Close" id="analysis_close" class="icon" src="./images/dialog_close.svg"/>' +
      ' <img title="Close" id="analysis_call" class="icon" src="./images/toolbar_element_test.svg"/>' +
      '</div>');
    ...
    $("#analysis_call").on("click", function(){
      modelChecker.check(self._graph, self.editor.getValue()).then(function (resp) {
        let compiled = Hogan.compile($("#markdownTemplate").html());
        let output = compiled.render(resp).replace(/\"/gi, "");
    ...
    var editor = ace.edit(this.$source[0]), session = editor.getSession();
    this.editor = editor;
    ...
    modelChecker.check(self._graph, self.editor.getValue()).then(function (resp) {
      let compiled = Hogan.compile($("#markdownTemplate").html());
      let output = compiled.render(resp).replace('"', "");
      self.updateResultWithContent(output);
    });    
  }
  ...
  updateResultWithContent(_content) {
