...
try {
	(async () => {
		const browser = await puppeteer.launch().catch(function (err) {
			console.error("Pupeteer cannot be launched.");
        });
        
		const page = await browser.newPage().catch(function (err) {
			console.error("Pupeteer cannot open a page.");
        });	
        ...
        await page.setViewport({
			...
			console.error("Pupeteer cannot set the viewport.");
        });
		page.on('console', msg => {
        ...
        await page.goto('http://localhost:7400/designer', {waitUntil: 'networkidle0'}).then(async function (response){
				// fulfillment
				...
				if (response.ok()) {
					...
					await page.evaluate((_json, _pkg) => {
						var json = _json;
						var pkg = _pkg;
						//app.fileNew();                                   
                    ...
                    .then(async (eval_results) => {
                        ...
                        console.error("Could not evaluate code in page: " + err);
                        await page.close();
                        await browser.close();
                    });
				} else {
